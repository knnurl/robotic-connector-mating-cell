    ...
    double z = 0.00, double distance = 0.05) // Default: Move by 0.05m
{
    ...
    if (x > 0.1) {
        target_pose.position.x += x / 2;
        RCLCPP_INFO(LOGGER, "X exceeded positive threshold. Moving forward...");
    } else if (x < -0.1) {
        target_pose.position.x += x / 2;
        RCLCPP_INFO(LOGGER, "X exceeded negative threshold. Moving backward...");
    }

    if (y > 0.1) {
        target_pose.position.y += y / 2;
        ...
        : Node("cam_subscriber"), move_group_(move_group)
    {
        ...
        cam_subscription_ = this->create_subscription<geometry_msgs::msg::Point>(
            ...
            [this](geometry_msgs::msg::Point::SharedPtr msg) {
                ...
                if (std::abs(x) > 0.1 || std::abs(y) > 0.1 || z > 0.55 || z < 0.45) {
                    RCLCPP_INFO(LOGGER, "Values outside thresholds. Triggering motion...");
                    move(move_group_, x, y, z, 0.04); // Move with adjustments
                } else {
